...
void timerCallback(void *pArg) {
  ...
  if(soundType!=SOUND_NONE && (effectIX>-1)){
    if(effectTmr==0){
      ...
      if(effectPos>=song[effectIX].length){
        effectIX=-1;
        if((soundType==SOUND_ALL) && (songIX>-1)){
          if(songFreq==0){
  ...
  if((soundType==SOUND_ALL) && (songIX>-1)){
    if(songTmr==0){
      songTmr=song[songIX].times[songPos]*4;
      songFreq=song[songIX].notes[songPos++];
      if(songPos>=song[songIX].length)
        songPos=0;
      if(effectIX==-1){
        if(songFreq==0){
          pinMode(BUZZER,INPUT);
